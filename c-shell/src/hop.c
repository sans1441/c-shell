...
void frecencyFilePath(ShellState *state, char *buf, size_t size) { snprintf(buf, size, "%s/.hop_frecency", state->home); }
...
HopType getHopType(char *arg) {
    if (strcmp(arg, "~") == 0) return HOP_HOME;

    if (strcmp(arg, ".") == 0) return HOP_CURRENT;

    if (strcmp(arg, "..") == 0) return HOP_PARENT;

    if (strcmp(arg, "-") == 0) return HOP_PREVIOUS;
...
void goToDirectory(char *path, ShellState *state) {
    ...
    if (getcwd(newcwd, sizeof(newcwd))) updateFrecency(newcwd, state);
}
...
void hop(Token *tokens, int token_count, ShellState *state) {
    ...
    for (int i = 1; i < token_count; i++) {
        ...
        case HOP_PREVIOUS:
            if (state->has_previous) goToDirectory(state->previous_dir, state);
            break;
